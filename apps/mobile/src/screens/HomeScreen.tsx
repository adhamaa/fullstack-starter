    ...
    <SafeAreaView className="flex-1 bg-bg">
      <ScrollView contentContainerClassName="grow items-center justify-center p-6">
        <View className="w-full rounded-3xl bg-bg-elevated p-6">
          ...
          {signOutWarning && <Text className="mt-3 text-sm text-warn-fg">{signOutWarning}</Text>}
          ...
          {me && (
            <View className="mt-4 gap-2">
              <View className="rounded-xl bg-bg-muted p-3">
                <Text className="text-xs uppercase tracking-wider text-ink-subtle">User ID</Text>
                <Text className="font-mono text-[#bae6fd]">{me.id}</Text>
              </View>
              {me.email && (
                <View className="rounded-xl bg-bg-muted p-3">
                  <Text className="text-xs uppercase tracking-wider text-ink-subtle">Email</Text>
