...
const ISSUER =
  process.env.EXPO_PUBLIC_KEYCLOAK_ISSUER ?? "http://localhost:8080/realms/fullstack";
...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      await updateTokens({
        ...
        accessTokenExpiresAt:
          Date.now() + (exchange.expiresIn ?? 60) * 1000
      });
    } catch (error) {
  ...
  const signOut = useCallback(async () => {
    if (discovery && tokens?.refreshToken) {
      try {
        await revokeAsync(
          { token: tokens.refreshToken, clientId: CLIENT_ID },
          discovery
        );
      } catch {
        // best-effort
