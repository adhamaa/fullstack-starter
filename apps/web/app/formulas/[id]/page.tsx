...
import { type FormulaDetails, formulasApi, type RadionicRate } from '../../../lib/services'
    ...
    <PageShell>
      <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-700 bg-bg-elevated p-6">
          ...
          {formula.body_system && (
            <p className="mt-2">
              <strong>Body system:</strong> {formula.body_system}
            </p>
          )}
          {formula.category && (
            <p>
        ...
        <section className="rounded-2xl border border-slate-700 bg-bg-elevated p-6">
          {tab === 'components' && (
            <div className="space-y-3">
              {formula.remedies?.length ? (
                formula.remedies.map((component) => (
                  <div key={component.remedy_id} className="rounded-lg bg-bg-muted p-3">
                    <Link
                      href={`/remedies/${component.remedy_id}`}
                      className="font-semibold text-accent"
                    >
                      {component.remedy_name}
                    </Link>
                    {component.proportion && (
                      <p className="text-sm text-ink-muted">Proportion: {component.proportion}</p>
                    )}
                  </div>
                ))
              ) : (
